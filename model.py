...
@ex.capture
def adaptive_spatial_mask(joints, mask_joint):
    joints = joints.numpy()
    all_joint = []
    number_missing_joint = np.sum(joints, axis=0)
    if np.sum(number_missing_joint) == 0:
        # Degree Centrality
        ...
        frequency_degree = ((number_missing_joint - number_missing_joint.min()) * (1/(number_missing_joint.max() - number_missing_joint.min() + 0.001) * 3)+1).astype(np.uint8)
        for i in range(25):
            all_joint += [i]*frequency_degree[i]
